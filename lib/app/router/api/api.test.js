...
describe('Server API HTTP Requests', () => {
  ...
  test('Should be able to delete a Link resource using an ID', async () => {
    let testToken = jwt.sign({ sub: 'test-id' }, process.env.TEST_SIGNATURE);
    let response = await mockRequest.delete(`/api/link/${testRecord.id}`).auth(testToken, { type: 'bearer' });
